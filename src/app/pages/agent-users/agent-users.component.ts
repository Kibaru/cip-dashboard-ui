...
export class AgentUsersComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.agentUsersSubscription = this.dataStore.agent_users.subscribe((data)=>{
      this.users = data;
      this.rerender();
    });
  ...
  deleteAgentUser(user_id:number , name:string){
    ...
    this.api.deleteAgentUser(user_id).subscribe((data)=>{
      this.api.getAgentUsers(this.dataStore.dashboardInfo.getValue().agent.id)
            .subscribe((data: any) => {
              this.dataStore.setAgentUsers(data);
              this.ngxService.stop();
  ...
  async refreshAgentUsers(){
    ...
    if(this.dataStore.dashboardInfo.getValue().agent != undefined){
